...
import { listActionPlans, updateActionPlan, deleteActionPlan, verifyActionPlanCompletion } from "@/lib/api"
...
export default function ActionPlansPage() {
  ...
  const [verificationResult, setVerificationResult] = useState<{ planId: string; success: boolean; message: string } | null>(null)
  ...
  const handleVerifyPlan = async (planId: string) => {
    setVerifyingId(planId)
    setVerificationResult(null)
    try {
      const res = await verifyActionPlanCompletion(planId)
      if (res.success) {
        ...
        setVerificationResult({
          planId,
          success: false,
          message: res.message || "Verification failed. Please check your changes and try again."
        })
      }
    } catch (err: any) {
      setVerificationResult({
        ...
        <div className="p-4 border-b border-white/5 space-y-3 bg-white/[0.02]">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              ...
              <option value="verified" className="bg-[#08080f]">Verified</option>
          ...
          <div className="flex-grow p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                {/* Boardroom Diagnosis Panel */}
                {(selectedPlan.signal_key || selectedPlan.verification_steps) && (
                  <div className="glass-panel border border-white/5 rounded-2xl p-5 space-y-4 shadow-lg bg-white/[0.02] relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full blur-3xl -z-10 pointer-events-none" />
                    
                    <div className="flex items-center justify-between border-b border-white/5 pb-3">
                      <div className="flex items-center gap-2">
                        <Sparkles className="w-4 h-4 text-primary animate-pulse" />
                        <h3 className="font-mono text-[10px] text-primary font-bold uppercase tracking-wider">
                          Boardroom Diagnosis Details
                        </h3>
                      </div>
                      {selectedPlan.priority_score !== undefined && (
                        <span className="text-[10px] font-mono text-muted-foreground bg-white/5 px-2.5 py-0.5 rounded-full border border-white/10">
                          Priority Score: <span className="text-white font-semibold">{selectedPlan.priority_score}/10</span>
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                      <div>
                        <span className="text-muted-foreground text-[10px] font-mono uppercase block mb-1">Source Signal</span>
                        <span className="text-white font-mono bg-white/5 px-2 py-1 rounded border border-white/10 text-[10px]">
                          {selectedPlan.signal_key || "generic_signal"}
                        </span>
                      </div>
                      <div>
                        <span className="text-muted-foreground text-[10px] font-mono uppercase block mb-1">Owner Label</span>
                        <span className="text-white font-sans text-xs">
                          {selectedPlan.owner_label || "SEO Specialist"}
                        </span>
                      </div>
                    </div>
                    ...
                    {selectedPlan.evidence && (
                      <div>
                        <span className="text-muted-foreground text-[10px] font-mono uppercase block mb-1">Evidence & Context</span>
                        <pre className="bg-[#08080f]/60 border border-white/5 rounded-lg p-3 text-[10px] font-mono text-emerald-400 overflow-x-auto leading-relaxed max-w-full whitespace-pre-wrap">
                          {typeof selectedPlan.evidence === "string" 
                            ? selectedPlan.evidence 
                            : JSON.stringify(selectedPlan.evidence, null, 2)}
                        </pre>
                      </div>
                    )}

                    {selectedPlan.implementation_steps && (
                      <div>
                        <span className="text-muted-foreground text-[10px] font-mono uppercase block mb-1">Implementation Steps</span>
                        <p className="text-white/90 leading-relaxed font-sans text-xs bg-white/[0.01] border border-white/5 rounded-lg p-3">
                          {selectedPlan.implementation_steps}
                        </p>
                      </div>
                    )}

                    {selectedPlan.verification_steps && (
                      <div>
                        <span className="text-muted-foreground text-[10px] font-mono uppercase block mb-1">Verification Steps</span>
                        <p className="text-white/90 leading-relaxed font-sans text-xs bg-white/[0.01] border border-white/5 rounded-lg p-3">
                          {selectedPlan.verification_steps}
                        </p>
                      </div>
                    ...
                    <div className="border-t border-white/5 pt-4 flex flex-col gap-3">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                          <h4 className="text-white text-xs font-semibold">Verify Implementation</h4>
                          <p className="text-[10px] text-muted-foreground">
                            Trigger an automated checker to verify that the fix is live and correct on the website.
                          </p>
                        </div>
                        ...
                        {selectedPlan.status === "verified" ? (
                          ...
                          <button
                            onClick={() => handleVerifyPlan(selectedPlan.id)}
                            disabled={verifyingId === selectedPlan.id}
                            className="px-4 py-2 bg-primary hover:bg-primary/95 text-black font-semibold rounded-lg text-[10px] font-mono uppercase tracking-wider transition-all duration-300 shadow-md flex items-center gap-2 disabled:opacity-50 disabled:pointer-events-none self-start sm:self-auto"
                          >
                            {verifyingId === selectedPlan.id ? (
                              <>
                      ...
                      {/* Verification Banner Alerts */}
                      {verificationResult && verificationResult.planId === selectedPlan.id && (
                        <div className={`p-3.5 rounded-lg border text-xs leading-relaxed flex items-start gap-2.5 animate-fade-in ${
                          ...
                          <div className="space-y-1">
                            <span className="font-semibold block font-mono text-[9px] uppercase tracking-wider">
                              {verificationResult.success ? "Verification Passed" : "Verification Failed"}
                            </span>
                            <p className="font-sans text-[11px] text-white/90">
                ...
                <h3 className="font-mono text-[10px] text-primary font-bold uppercase tracking-wider flex items-center gap-2 pt-2">
                  <ClipboardList className="w-4 h-4 text-primary" />
